...
class UserController extends Controller
{
   /**
    ...
    public function index(){
        $users=User::all()->load('Rol','Compania');
        if(is_object($users)){
            $data=array(
                'code'=>200,
                'status'=>'success',
    ...
    public function show($id){
        $users=MaterialMayor::where('compania_id',$id)->get();
        if(is_object($users)){
            $data=array(
                'code'=>200,
                'status'=>'success',
    ...
    public function register(Request $request){
        ...
        if(!empty($params) && !empty($params_array)){
            ...
            if($validate->fails()){
                ...
                $user->rut=$params_array['rut'];
                $user->direccion=$params_array['direccion'];
                $user->telefono=$params_array['telefono'];
                $user->talla_calzado=$params_array['talla_calzado'];
                $user->talla_ropa=$params_array['talla_ropa'];
                $user->cargo=$params_array['cargo'];
                $user->servicio=$params_array['servicio'];
                $user->compania_id=$params_array['compania_id'];
    ...
    public function roleA($id){

        //conseguir el registro 
        $user=User::where('id',$id)->update(['rol_id'=>'1']);
    ...
    public function roleB($id){

        //conseguir el registro 
        $user=User::where('id',$id)->update(['rol_id'=>'3']);
    ...
    public function roleO($id){

        //conseguir el registro 
        $user=User::where('id',$id)->update(['rol_id'=>'2']);
